from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field
from typing_extensions import Self

from ..types import UNSET, Unset

T = TypeVar("T", bound="VlanNetworkAffectingSsidVO")


@_attrs_define
class VlanNetworkAffectingSsidVO:
    """
    Attributes:
        id (str | Unset): SSID ID
        name (str | Unset): SSID name
        wlan_id (str | Unset): WLAN group ID
        wlan_name (str | Unset): WLAN group name
    """

    id: str | Unset = UNSET
    name: str | Unset = UNSET
    wlan_id: str | Unset = UNSET
    wlan_name: str | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        id = self.id

        name = self.name

        wlan_id = self.wlan_id

        wlan_name = self.wlan_name

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if id is not UNSET:
            field_dict["id"] = id
        if name is not UNSET:
            field_dict["name"] = name
        if wlan_id is not UNSET:
            field_dict["wlanId"] = wlan_id
        if wlan_name is not UNSET:
            field_dict["wlanName"] = wlan_name

        return field_dict

    @classmethod
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        d = dict(src_dict)
        id = d.pop("id", UNSET)

        name = d.pop("name", UNSET)

        wlan_id = d.pop("wlanId", UNSET)

        wlan_name = d.pop("wlanName", UNSET)

        vlan_network_affecting_ssid_vo = cls(
            id=id,
            name=name,
            wlan_id=wlan_id,
            wlan_name=wlan_name,
        )

        vlan_network_affecting_ssid_vo.additional_properties = d
        return vlan_network_affecting_ssid_vo

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
