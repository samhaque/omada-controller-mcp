...
T = TypeVar("T", bound="UpdateApPowerSavingConfigOpenApiVO")


@_attrs_define
class UpdateApPowerSavingConfigOpenApiVO:
    """
    Attributes:
        mode (int | Unset): Parameter [mode] is Power Saving Mode of device and should not be null. 0: OFF, 1: Standard(
            based on the user‑specified time and/or band settings ), 2: Smart, 3: Deep Power‑Saving
        time_enable (bool | Unset): Parameter [timeEnable] can be TRUE only when [mode] = 1. Power Saving trigger by
            time config status. True: enable, false: disable.
        start_time_h (int | Unset): Start time of trigger by time(unit: hour); It should be within the range of 0–23;
            This field is required when Parameter [timeEnable] is true.
        start_time_m (int | Unset): Start time of trigger by time(unit: minute); It should be within the range of 0–59;
            This field is required when Parameter [timeEnable] is true.
        end_time_h (int | Unset): End time of trigger by time(unit: hour); It should be within the range of 0–23; This
            field is required when Parameter [timeEnable] is true.
        end_time_m (int | Unset): End time of trigger by time(unit: minute); It should be within the range of 0–59; This
            field is required when Parameter [timeEnable] is true.
        band_enable (bool | Unset): Parameter [bandEnable] can be TRUE only when [mode] = 1. Power Saving trigger by
            band config status. True: enable, false: disable.
        bands (list[int] | Unset): Select bands list config of trigger by band;It should be a value as follows: 0:
            2.4GHz; 1: 5GHz; 2: 5G2Hz; 3: 6GHz;This field is required when Parameter [bandEnable] is true;Please note that
            the band filled in must be supported by the device, otherwise the modified configuration will not take effect
            properly
        idle_duration (int | Unset): Idle duration config of trigger by band(unit: minute); It should be within the
            range of 60–1440; This field is required when Parameter [bandEnable] is true.
    """
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        update_ap_power_saving_config_open_api_vo = cls(
            mode=mode,
        ...
        update_ap_power_saving_config_open_api_vo.additional_properties = d
        return update_ap_power_saving_config_open_api_vo
