...
if TYPE_CHECKING:
    from ..models.check_wan_lan_status_open_api_vo_port_name import (
        CheckWanLanStatusOpenApiVOPortName,
    )
    from ..models.wan_list import WanList


T = TypeVar("T", bound="CheckWanLanStatusOpenApiVO")


@_attrs_define
class CheckWanLanStatusOpenApiVO:
    """
    Attributes:
        port_name (CheckWanLanStatusOpenApiVOPortName | Unset): The map of port name, key: "port ID", value: "port name"
        network_comptent (int | Unset): NetworkComptent
        adopted_gateway (bool | Unset): Indicates whether the gateway is connected
        support_ipv_6 (int | Unset): IIndicates whether the gateway support IPv6
        wan_list (list[WanList] | Unset): The WAN list of gateway
        iptv_ports (list[str] | Unset): IPTVPorts
        pre_osg_model (int | Unset): PreOSGModel
        model (str | Unset): Model of device,for example:EAP225
        model_version (str | Unset): Model version of device,for example:3.0
        has_gateway (bool | Unset): Whether adopt a gateway or enable WAN Settings Overrides.
    """

    port_name: CheckWanLanStatusOpenApiVOPortName | Unset = UNSET
    network_comptent: int | Unset = UNSET
    adopted_gateway: bool | Unset = UNSET
    support_ipv_6: int | Unset = UNSET
    wan_list: list[WanList] | Unset = UNSET
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        port_name: CheckWanLanStatusOpenApiVOPortName | Unset
        if isinstance(_port_name, Unset):
            port_name = UNSET
        else:
            port_name = CheckWanLanStatusOpenApiVOPortName.from_dict(_port_name)
        ...
        if _wan_list is not UNSET:
            ...
            for wan_list_item_data in _wan_list:
                wan_list_item = WanList.from_dict(wan_list_item_data)
        ...
        check_wan_lan_status_open_api_vo = cls(
            port_name=port_name,
        ...
        check_wan_lan_status_open_api_vo.additional_properties = d
        return check_wan_lan_status_open_api_vo
