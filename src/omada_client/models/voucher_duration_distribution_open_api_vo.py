from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field
from typing_extensions import Self

from ..types import UNSET, Unset

T = TypeVar("T", bound="VoucherDurationDistributionOpenApiVO")


@_attrs_define
class VoucherDurationDistributionOpenApiVO:
    """
    Attributes:
        duration (int | Unset): Duration of single voucher, unit: minutes
        duration_type (int | Unset): The duration type of the voucher. It should be a value as follows: 0: Client
            duration, each client expires after the duration is used. 1: Voucher duration, after reaching the voucher
            duration, clients using the voucher will expire
        total_duration (int | Unset): Total duration of vouchers, unit: minutes
        used_count (int | Unset): Used count of vouchers
    """

    duration: int | Unset = UNSET
    duration_type: int | Unset = UNSET
    total_duration: int | Unset = UNSET
    used_count: int | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        duration = self.duration

        duration_type = self.duration_type

        total_duration = self.total_duration

        used_count = self.used_count

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if duration is not UNSET:
            field_dict["duration"] = duration
        if duration_type is not UNSET:
            field_dict["durationType"] = duration_type
        if total_duration is not UNSET:
            field_dict["totalDuration"] = total_duration
        if used_count is not UNSET:
            field_dict["usedCount"] = used_count

        return field_dict

    @classmethod
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        d = dict(src_dict)
        duration = d.pop("duration", UNSET)

        duration_type = d.pop("durationType", UNSET)

        total_duration = d.pop("totalDuration", UNSET)

        used_count = d.pop("usedCount", UNSET)

        voucher_duration_distribution_open_api_vo = cls(
            duration=duration,
            duration_type=duration_type,
            total_duration=total_duration,
            used_count=used_count,
        )

        voucher_duration_distribution_open_api_vo.additional_properties = d
        return voucher_duration_distribution_open_api_vo

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
