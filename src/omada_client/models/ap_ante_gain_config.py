...
if TYPE_CHECKING:
    from ..models.ap_radio_ante_gain_config_open_api_vo import (
        ApRadioAnteGainConfigOpenApiVO,
    )


T = TypeVar("T", bound="ApAnteGainConfig")


@_attrs_define
class ApAnteGainConfig:
    """
    Attributes:
        ante_gain_list (list[ApRadioAnteGainConfigOpenApiVO] | Unset): Antenna gain list.
    """

    ante_gain_list: list[ApRadioAnteGainConfigOpenApiVO] | Unset = UNSET
    ...
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        ...
        if _ante_gain_list is not UNSET:
            ...
            for ante_gain_list_item_data in _ante_gain_list:
                ante_gain_list_item = ApRadioAnteGainConfigOpenApiVO.from_dict(
                    ante_gain_list_item_data
                )

                ante_gain_list.append(ante_gain_list_item)

        ap_ante_gain_config = cls(
            ante_gain_list=ante_gain_list,
        )

        ap_ante_gain_config.additional_properties = d
        return ap_ante_gain_config
