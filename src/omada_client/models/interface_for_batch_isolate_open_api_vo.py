from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field
from typing_extensions import Self

from ..types import UNSET, Unset

T = TypeVar("T", bound="InterfaceForBatchIsolateOpenApiVO")


@_attrs_define
class InterfaceForBatchIsolateOpenApiVO:
    """
    Attributes:
        name (str | Unset): Lan network name.
        id (str | Unset): Lan network Id.
        vlan (int | Unset): vlan Id.
        vlans (str | Unset): multiple vlan Ids
        isolation (bool | Unset): Whether isolate network.
    """

    name: str | Unset = UNSET
    id: str | Unset = UNSET
    vlan: int | Unset = UNSET
    vlans: str | Unset = UNSET
    isolation: bool | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        name = self.name

        id = self.id

        vlan = self.vlan

        vlans = self.vlans

        isolation = self.isolation

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if name is not UNSET:
            field_dict["name"] = name
        if id is not UNSET:
            field_dict["id"] = id
        if vlan is not UNSET:
            field_dict["vlan"] = vlan
        if vlans is not UNSET:
            field_dict["vlans"] = vlans
        if isolation is not UNSET:
            field_dict["isolation"] = isolation

        return field_dict

    @classmethod
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        d = dict(src_dict)
        name = d.pop("name", UNSET)

        id = d.pop("id", UNSET)

        vlan = d.pop("vlan", UNSET)

        vlans = d.pop("vlans", UNSET)

        isolation = d.pop("isolation", UNSET)

        interface_for_batch_isolate_open_api_vo = cls(
            name=name,
            id=id,
            vlan=vlan,
            vlans=vlans,
            isolation=isolation,
        )

        interface_for_batch_isolate_open_api_vo.additional_properties = d
        return interface_for_batch_isolate_open_api_vo

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
