from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field
from typing_extensions import Self

from ..types import UNSET, Unset

T = TypeVar("T", bound="UpgradeRes")


@_attrs_define
class UpgradeRes:
    """
    Attributes:
        finished (bool | Unset): Whether the task is complete
        unfinished_count (int | Unset): Total number of devices that have not been upgraded
    """

    finished: bool | Unset = UNSET
    unfinished_count: int | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        finished = self.finished

        unfinished_count = self.unfinished_count

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if finished is not UNSET:
            field_dict["finished"] = finished
        if unfinished_count is not UNSET:
            field_dict["unfinishedCount"] = unfinished_count

        return field_dict

    @classmethod
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        d = dict(src_dict)
        finished = d.pop("finished", UNSET)

        unfinished_count = d.pop("unfinishedCount", UNSET)

        upgrade_res = cls(
            finished=finished,
            unfinished_count=unfinished_count,
        )

        upgrade_res.additional_properties = d
        return upgrade_res

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
