from __future__ import annotations

from collections.abc import Mapping
from typing import TYPE_CHECKING, Any, TypeVar

from attrs import define as _attrs_define
from attrs import field as _attrs_field
from typing_extensions import Self

from ..types import UNSET, Unset

if TYPE_CHECKING:
    from ..models.device_cli_vo_variable_map import DeviceCliVOVariableMap


T = TypeVar("T", bound="DeviceCliVO")


@_attrs_define
class DeviceCliVO:
    """List of devices bound to the CLI configuration.

    Attributes:
        variable_map (DeviceCliVOVariableMap | Unset): The values of different user-defined variables.
        device_mac (str | Unset): Device mac
        device_name (str | Unset): Device name
        device_model (str | Unset): Model of device
        model_version (str | Unset): Model version of device
    """

    variable_map: DeviceCliVOVariableMap | Unset = UNSET
    device_mac: str | Unset = UNSET
    device_name: str | Unset = UNSET
    device_model: str | Unset = UNSET
    model_version: str | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        variable_map: dict[str, Any] | Unset = UNSET
        if not isinstance(self.variable_map, Unset):
            variable_map = self.variable_map.to_dict()

        device_mac = self.device_mac

        device_name = self.device_name

        device_model = self.device_model

        model_version = self.model_version

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({})
        if variable_map is not UNSET:
            field_dict["variableMap"] = variable_map
        if device_mac is not UNSET:
            field_dict["deviceMac"] = device_mac
        if device_name is not UNSET:
            field_dict["deviceName"] = device_name
        if device_model is not UNSET:
            field_dict["deviceModel"] = device_model
        if model_version is not UNSET:
            field_dict["modelVersion"] = model_version

        return field_dict

    @classmethod
    def from_dict(cls, src_dict: Mapping[str, Any]) -> Self:
        from ..models.device_cli_vo_variable_map import (
            DeviceCliVOVariableMap,
        )

        d = dict(src_dict)
        _variable_map = d.pop("variableMap", UNSET)
        variable_map: DeviceCliVOVariableMap | Unset
        if isinstance(_variable_map, Unset):
            variable_map = UNSET
        else:
            variable_map = DeviceCliVOVariableMap.from_dict(_variable_map)

        device_mac = d.pop("deviceMac", UNSET)

        device_name = d.pop("deviceName", UNSET)

        device_model = d.pop("deviceModel", UNSET)

        model_version = d.pop("modelVersion", UNSET)

        device_cli_vo = cls(
            variable_map=variable_map,
            device_mac=device_mac,
            device_name=device_name,
            device_model=device_model,
            model_version=model_version,
        )

        device_cli_vo.additional_properties = d
        return device_cli_vo

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
